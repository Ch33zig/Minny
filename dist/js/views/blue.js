  ...
  container.innerHTML = `
    <div class="blue-grid">
      ...
      <section class="panel col rule-col">
        <div class="col-head"><p class="section-label">ACCEPTED</p><span class="col-count mono">${accepted.length}</span></div>
        <div class="col-body">${accepted.map(rule).join('') || '<div class="empty">Nothing has cleared the gate.</div>'}</div>
      </section>

      <section class="panel col rule-col">
        <div class="col-head"><p class="section-label">REJECTED</p><span class="col-count mono">${rejected.length}</span></div>
...
function rule(r) {
  ...
  return `
    <article class="card rule ${passed ? 'rule-pass' : 'rule-fail'}">
      <div class="card-head">
        <span class="card-id mono">${esc(r.id || '')}</span>
        <div class="variant-tags">
          ${r.placeholder ? '<span class="op-chip mono none">fixture</span>' : ''}
          <span class="${passed ? 'check pass' : 'check fail'} mono">${passed ? 'ACCEPTED' : 'REJECTED'}</span>
        </div>
      </div>
      ...
      ${r.rationale ? `<p class="method"><span class="method-label mono">WHY THIS RULE</span>${esc(r.rationale)}</p>` : ''}

      <div class="dsl">
        <div class="dsl-head mono"><span>WHEN</span>${parse.ok === false ? '<span class="check fail">parse error</span>' : '<span class="mono dsl-ok">parses · depth ' + esc(parse.depth ?? '?') + ' · ' + esc(parse.nodes ?? '?') + ' nodes</span>'}</div>
        <code>${esc(r.when || '')}</code>
        ${r.explain ? `<div class="dsl-explain mono">${esc(r.explain)}</div>` : ''}
        ${parse.error ? `<div class="dsl-error mono">${esc(parse.error)}</div>` : ''}
        ${parse.grammar_note ? `<div class="dsl-note mono">${esc(parse.grammar_note)}</div>` : ''}
      </div>
...
function gateRow(key, v) {
  ...
  return `
    <div class="gate-row ${v.pass ? 'pass' : 'fail'}">
      ...
      <span class="gate-target mono">${esc(target)}</span>
      <span class="gate-measured mono">${esc(measured)}</span>
    </div>`;
}
...
function beforeAfter(ba) {
  ...
  const rows = Object.entries(ba).map(([key, v]) => {
    ...
    return `<div class="ba-row">
      <span class="ba-name mono">${esc(key.replace(/_/g, ' '))}</span>
      <span class="ba-val mono">${esc(before)}</span>
      <span class="ba-arrow mono">→</span>
      <span class="ba-val mono ${moved ? 'moved' : 'flat'}">${esc(after)}</span>
    </div>`;
  }).join('');
  return `<div class="ba"><div class="ba-head mono">BEFORE → AFTER</div>${rows}</div>`;
}
