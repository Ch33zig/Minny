    ...
    <div class="case-grid">
      ...
      <section class="panel col" id="actorsCol">
        <div class="col-head">
          ...
          <span class="col-count mono">2</span>
        </div>
        <div class="col-body">
          ${suspect(a, 'attacker')}
          ${suspect(v, 'victim')}
          <div class="col-head inner">
            ...
            <span class="col-count mono">${(cf.unknowns || []).length}</span>
          </div>
      ...
      <section class="panel col" id="findingsCol">
        <div class="col-head">
          ...
          <span class="col-count mono">${(cf.findings || []).length}</span>
        </div>
      ...
      <section class="panel col" id="timelineCol">
        <div class="col-head">
          ...
          <span class="col-count mono">${(cf.timeline || []).length}</span>
        </div>
      ...
      <section class="panel col" id="sideCol">
        <div class="col-head">
          ...
          <span class="col-count mono">${(cf.dismissed || []).length}</span>
        </div>
        <div class="col-body">
...
function suspect(actor, kind) {
  ...
  const stats = (actor.stats || []).map((s) =>
    `<div class="stat"><b class="mono">${esc(s.value)}</b><span>${esc(s.label)}</span></div>`).join('');
  return `
    <article class="suspect ${esc(kind)}">
      <div class="suspect-head">
        ...
        <div class="suspect-id">
          ...
          <div class="suspect-ip mono">${esc(actor.ip || 'no IP recorded')}</div>
        </div>
...
function finding(f) {
  ...
  return `
    <article class="card finding conf-edge-${esc(level)}">
      <div class="card-head">
        <span class="card-id mono">${esc(f.id || '')}</span>
        ${confChip(f.confidence)}
      </div>
      <p class="claim">${esc(f.claim)}</p>
      ${mailOnly ? '<p class="mail-only">Supported by mailbox records only.</p>' : ''}
      <p class="method"><span class="method-label mono">METHOD</span>${esc(f.method || '')}</p>
      ${f.query ? `<div class="query mono" title="A saved, re-runnable query in the forensics module">${esc(f.query)}</div>` : ''}
...
function dismissed(d) {
  return `
    <article class="card dismissed">
      <div class="card-head">
        <span class="card-id mono">${esc(d.id || '')}</span>
        <span class="cleared">CLEARED</span>
      </div>
      <p class="claim">${esc(d.lead)}</p>
      <p class="method"><span class="method-label mono">WHY IT IS NOTHING</span>${esc(d.why || '')}</p>
      ${d.query ? `<div class="query mono">${esc(d.query)}</div>` : ''}
...
function unknown(u) {
  return `
    <article class="card unknown">
      <div class="card-head">
        <span class="card-id mono">${esc(u.id || '')}</span>
